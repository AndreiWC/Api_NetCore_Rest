using System.Threading.Tasks;
using Api.Domain.Interfaces.Services.User;
using Moq;
using Xunit;

namespace Api.Service.Test.Usuario
{
    public class QuandoForExecutarUpdate : UsuarioTestes
    {
        //CRIA UMA OBJ PRIVADO DA INTERFACE IUSERSERVICE E PASSA PARA A MOCK
        private IUserService _services;
        //A MOCK ALOCA NA MEMOMRIA O OBJETO IUSERSERVICE COM TODAS AS FUNÇÕES PARA REALIZAR A ENTRADA E A SAIDA DE DADOS 
        private Mock<IUserService> _serviceMock;

        [Fact(DisplayName = "É Possível Executar o Método Update")]
        public async Task E_POSSIVEL_EXECUTAR_METODO_UPDATE()
        {
            //INSTANCIA A MOCK
            _serviceMock = new Mock<IUserService>();
            //CONFIGURA A MOCK COM UM OBJETO CRIADO NA CLASSE USUARIOTESTE, RETORNA O OBJETE userDtoCreate POR MEIO DO Post DO OBJETO
            _serviceMock.Setup(m => m.Post(userDtoCreate)).ReturnsAsync(userDtoCreateResult);
            //PASSA O OBJETO CRIADO PARA A SERVICE
            _services = _serviceMock.Object;

            //INICIO DOS TESTES - 1º
            var result = await _services.Post(userDtoCreate);
            Assert.NotNull(result);
            Assert.Equal(NomeUsuario, result.Name);
            Assert.Equal(EmailUsuario, result.Email);


            //INSTANCIA A MOCK
            _serviceMock = new Mock<IUserService>();
            //CONFIGURA A MOCK COM UM OBJETO CRIADO NA CLASSE USUARIOTESTE, RETORNA O OBJETE UserDtoUpdate POR MEIO DO Put DO OBJETO
            _serviceMock.Setup(m => m.Put(UserDtoUpdate)).ReturnsAsync(UserDtoUpdateResult);
            //PASSA O OBJETO CRIADO PARA A SERVICE
            _services = _serviceMock.Object;

            //INICIO DOS TESTES - 1º
            var resultUpdate = await _services.Put(UserDtoUpdate);
            Assert.NotNull(result);
            Assert.Equal(NomeUsuarioAlterado, resultUpdate.Name);
            Assert.Equal(EmailUsuarioAlterado, resultUpdate.Email);


        }
    }
}